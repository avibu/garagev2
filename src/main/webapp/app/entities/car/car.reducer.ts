...
export default (state: CarState = initialState, action): CarState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_CARS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_CARS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_CARS):
...
const apiSearchUrl = 'api/_search/cars';

// Actions

export const getSearchEntities: ICrudSearchAction<ICar> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_CARS,
  payload: axios.get<ICar>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
